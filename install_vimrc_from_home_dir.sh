...
def install_vim_configuration():
    """
    Installs the Vim configuration:
      - Writes the .vimrc file with the provided content.
      - Creates necessary directories (~/.vim/autoload and ~/.vim/bundle).
      - Downloads Pathogen to ~/.vim/autoload.
      - Clones Vundle into ~/.vim/bundle.
      - Runs Vim to install plugins via Vundle.
    """
    home = os.path.expanduser("~")
    vimrc_path = os.path.join(home, ".vimrc")
    vim_dir = os.path.join(home, ".vim")
    autoload_dir = os.path.join(vim_dir, "autoload")
    bundle_dir = os.path.join(vim_dir, "bundle")

    # The vimrc content you provided.
    vimrc_content = r'''------------------------------------------------------------------------------
...
'''
    ...
    print(f"Created .vimrc at {vimrc_path}")
    ...
    # Download Pathogen into autoload directory
    pathogen_url = "https://tpo.pe/pathogen.vim"
    pathogen_dest = os.path.join(autoload_dir, "pathogen.vim")
    try:
        print("Downloading Pathogen...")
        urllib.request.urlretrieve(pathogen_url, pathogen_dest)
        print(f"Pathogen downloaded to {pathogen_dest}")
    except Exception as e:
        print("Error downloading Pathogen:", e)

    # Clone Vundle into the bundle directory if not already present
    vundle_dir = os.path.join(bundle_dir, "Vundle.vim")
    if not os.path.exists(vundle_dir):
        try:
            print("Cloning Vundle...")
            subprocess.run(["git", "clone", "https://github.com/VundleVim/Vundle.vim.git", vundle_dir], check=True)
            print(f"Vundle cloned into {vundle_dir}")
        except subprocess.CalledProcessError as e:
            print("Error cloning Vundle:", e)
    else:
        print("Vundle is already installed.")

    # Run Vim to install plugins via Vundle (this requires Vim to be installed)
    try:
        print("Running Vim to install plugins (this might take a moment)...")
        subprocess.run(["vim", "+PluginInstall", "+qall"], check=True)
        print("Vundle plugins installed successfully.")
    except subprocess.CalledProcessError as e:
        print("Error running Vim for PluginInstall:", e)
